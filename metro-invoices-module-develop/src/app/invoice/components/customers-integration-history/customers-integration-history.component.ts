...
import { CustomerIntegrationHistoryService } from './../../services/customer-integration-history.service';
...
@Component({
  selector: 'app-customers-integration-history',
  templateUrl: './customers-integration-history.component.html',
  styleUrls: ['./customers-integration-history.component.scss']
})


export class CustomersIntegrationHistoryComponent implements OnInit {

  cols = [
    { title: 'Arquivo', prop: 'fileDTO' },
    { title: 'Data de criação', prop: 'createdOn' },
  ...
  constructor(
      ...
      private customerIntegrationHistoryService: CustomerIntegrationHistoryService,
  ...
  executeAction(event: any) {
    if (event.action.action == 'download_file') {
      if (event.line.integrationOrigin == "Oracle") {
        this.fileService.downloadFileFTP(event.line.fileDTO, 'GF')
            .subscribe(
                content => {
        ...
        this.customerIntegrationHistoryService.downloadFile(event.line.fileHash)
            .subscribe(
                content => {
                  BlobUtil.startDownload(
                      event.line.fileDTO,
      ...
      this.customerIntegrationService.processFile(event.line.id)
        .subscribe(
          (resp) => {
  ...
  public renderFiles() {
    ...
    this.customerIntegrationHistoryService.getFileAutoComplete(autoComplete)
        .pipe(finalize(() => {
