...
import { Pageable, ArrayPage } from 'src/app/shared/util/pageable';
...
@Component({
  selector: 'app-situation-import',
  templateUrl: './situation-import.component.html',
  styleUrls: ['./situation-import.component.scss']
})
export class SituationImport implements OnInit {
  ...
  constructor(private messageService: MessageService,
    ...
    private integrationFileStatusService: IntegrationFileStatusService) {
  }
  ...
  uploadFile() {
    ...
    this.integrationFileStatusService.uploadFileStatus(file).subscribe(resp => {
      this.hideSpinner = true;
      this.invoicesSituationList = resp.filter(r => r.idInvoice);
  ...
  uploadFileText() {
    ...
    this.hideSpinner = false;
    fr.onload = (e) => {
      this.integrationFileStatusService.uploadFileStatusText(fr.result, file.name).subscribe(resp => {
        this.hideSpinner = true;
        this.invoicesSituationList = resp.filter(r => r.idInvoice);
        this.onPageEventCustomerPeriod({pageIndex: 0, pageSize: 10});
        const foundMessage = resp.find(element => element.resultImportMessage);
