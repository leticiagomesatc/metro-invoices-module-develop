...
export class CustomerIntegrationHistoryService extends FwSearchService {
  ...
  getFileAutoComplete(type: string): Observable<any> {
    return type && type.length > 0 ?
      this.http.get(environment.invoicesApi + `customers-integration-history/filename-autocomplete/${type}`) :
      this.http.get(environment.invoicesApi + 'customers-integration-history/filename-autocomplete');
  }
  ...
  downloadFile(fileHash: number): Observable<any> {
    ...
    return this.http.get(environment.invoicesApi + 'customers-integration-history/download-file/' + fileHash, { headers: headers, responseType: 'blob' });
  }

  search(filters: any, pageable: any, sort: any): Observable<ArrayPage<any>> {
    return super.search('customers-integration-history', filters, pageable, sort);
  }
